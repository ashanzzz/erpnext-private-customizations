...
clean_script = """
// 1. Permanently remove and hide legacy DOM injection container
(function() {
    function remove_legacy_dom() {
    ...
    remove_legacy_dom();
    if (typeof $ !== 'undefined') {
        $(document).ready(remove_legacy_dom);
        $(document).on('app_ready page-change route-change toolbar_setup', remove_legacy_dom);
    }
    setInterval(remove_legacy_dom, 300);
})();
...
res = call_api('/api/resource/Website%20Script/Website%20Script', method='PUT', data={"javascript": clean_script})
print("Updated Website Script to permanently purge legacy DOM container:", res is not None)
