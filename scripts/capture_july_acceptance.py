...
def run_july_capture():
    with sync_playwright() as p:
        ...
        # 3. 切换到 7 月 (真实发票所属期 2026-07)
        page.select_option("#sel-month", "7")
        time.sleep(3)

        # 4. 保存草稿以固化 7 月单证
        btn_save = page.locator("#btn-save-draft")
        if btn_save.count() > 0 and btn_save.is_visible():
            btn_save.click()
            time.sleep(2)


        # 5. 截图工作台 (7月份)
        ss_wb = os.path.join(ARTIFACT_DIR, "live_acceptance_lease_workbench_july.png")
        page.screenshot(path=ss_wb, full_page=True)
        print(f"Saved 7月 Workbench: {ss_wb}")

        # 6. 打开单证预览弹窗（天津祺富机械加工有限公司）
        ...
        if btn_print.count() > 0:
            ...
            print(f"Saved 7月 Bill Dialog: {ss_bill}")

        browser.close()
        print("July capture complete!")
