...
def run_acceptance():
    print("Launching Playwright for final acceptance...")
    with sync_playwright() as p:
        ...
        time.sleep(3)
        page.wait_for_selector(".list-row-container", timeout=10000)
        ss1 = os.path.join(ARTIFACT_DIR, "live_acceptance_property_lease_list.png")
        page.screenshot(path=ss1)
        print(f"Saved: {ss1}")

        # 3. 点击列表第一行或 930平米 仓库进入 Form
        print("3. Clicking first row in list...")
        first_row = page.locator(".list-row-container .list-row").first
        if first_row.count() > 0:
            first_row.click()
            time.sleep(3)
            page.wait_for_selector(".form-page", timeout=10000)
        ...
        btn_print = page.locator("#btn-preview-bills, #btn-print-total").first
        ...
        browser.close()
        print("Final acceptance capture complete!")
