...
# Clear cache
try:
    cc_req = urllib.request.Request(f"{SITE_URL}/api/method/frappe.handler.clear_user_cache")
    opener.open(cc_req)
    print("User cache cleared successfully.")
except Exception as e:
    print("Cache clear error:", e)

# Let's check get_website_user_home_page / login
login_req2 = urllib.request.Request(
    f"{SITE_URL}/api/method/login",
    data=json.dumps({'usr': USER, 'pwd': PWD}).encode('utf-8'),
    headers={'Content-Type': 'application/json'}
)
resp2 = opener.open(login_req2)
print("Login response after cache clear:", resp2.read().decode('utf-8'))
