...
JS_CONTENT = r'''(function () {
  ...
  /*
    场景2-采购发票：统计“未写报销申请”的 Purchase Invoice (差集)
  */
...
# 确保在 Home Workspace 中挂载
home_ws = frappe.get_doc("Workspace", "Home")
has_cb_child = any(cb.custom_block_name == BLOCK_NAME for cb in (home_ws.custom_blocks or []))
if not has_cb_child:
    home_ws.append("custom_blocks", {{
        "custom_block_name": BLOCK_NAME,
        "label": BLOCK_NAME
    }})

content_list = json.loads(home_ws.content or "[]")
has_content_item = any(item.get("type") == "custom_block" and item.get("data", {{}}).get("custom_block_name") == BLOCK_NAME for item in content_list)
...
frappe.db.commit()
frappe.clear_cache()
print("[OK] Refactored Permission-Aware Custom HTML Block deployed to DB!")
"""

sftp = ssh.open_sftp()
with sftp.open('/tmp/deploy_refactored_chb.py', 'wb') as f:
    f.write(py_code.encode('utf-8'))

cmd1 = "docker cp /tmp/deploy_refactored_chb.py erpnext16:/tmp/deploy_refactored_chb.py"
cmd2 = "docker exec -u frappe -w /home/frappe/frappe-bench/sites erpnext16 /home/frappe/frappe-bench/env/bin/python3 /tmp/deploy_refactored_chb.py"
