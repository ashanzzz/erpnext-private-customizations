...
async def run():
    async with async_playwright() as p:
        ...
        fields_info = await page.evaluate("""() => {
            ...
            return grid.docfields.map(d => ({ fn: d.fieldname, label: d.label, type: d.fieldtype, hidden: d.hidden }));
        }""")
        print("DOCFIELDS IN BROWSER:")
        for df in fields_info:
            if not df['hidden']:
                print("  VISIBLE:", df)
            elif df['fn'] in ['description', 'custom_spec_model']:
                print("  HIDDEN:", df)
