...
async def run():
    async with async_playwright() as p:
        ...
        # Direct navigation to /desk/my-business
        ...
        res = await page.evaluate("""() => {
            if (frappe.app && frappe.app.sidebar) {
                ...
                return {
                    ...
                    items_len: frappe.app.sidebar.workspace_sidebar_items?.length || 0,
                    dom_items_len: $('.body-sidebar .standard-sidebar-item').length
                };
            }
            return null;
        }""")
        print("After setup('业务扩展'):", res)
        await asyncio.sleep(2)

        shot1 = os.path.join(OUTPUT_DIR, "after_explicit_sidebar_setup.png")
        ...
        print(f"DOM items ({len(dom_items)}):", dom_items[:10])
