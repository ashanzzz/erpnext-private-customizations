...
ws_list = call_api('/api/resource/Workspace?limit_page_length=100&fields=["name","title","module","parent_page"]')
if ws_list and 'data' in ws_list:
    print(f"Total Workspaces ({len(ws_list['data'])}):")
    for w in ws_list['data']:
        print(f"  - {w.get('title')} ({w.get('name')}) [Module: {w.get('module')}, Parent: '{w.get('parent_page')}']")
