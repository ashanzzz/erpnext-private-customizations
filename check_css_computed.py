...
async def run():
    async with async_playwright() as p:
        ...
        styles = await page.evaluate("""() => {
            const sec = document.querySelector('.grid-row-open .form-section:nth-child(2) .section-body');
            const col = document.querySelector('.grid-row-open .form-section:nth-child(2) .form-column');
            return {
                secDisplay: sec ? window.getComputedStyle(sec).display : null,
                colDisplay: col ? window.getComputedStyle(col).display : null,
