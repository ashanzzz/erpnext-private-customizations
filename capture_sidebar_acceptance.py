...
async def run():
    async with async_playwright() as p:
        ...
        # 1. Landing on /desk/my-business
        await page.goto(f"{SITE_URL}/desk/my-business", wait_until="networkidle", timeout=30000)
        await asyncio.sleep(4)

        shot1 = os.path.join(OUTPUT_DIR, "sidebar_fixed_my_business.png")
        await page.screenshot(path=shot1)
        print(f"Screenshot 1 (My Business): {shot1}")

        # Check total sidebar items count
        items = await page.eval_on_selector_all(".body-sidebar .standard-sidebar-item", "els => els.map(e => e.textContent.trim())")
        print(f"Total sidebar items found: {len(items)}")

        # 2. Test root navigation
        await page.goto(f"{SITE_URL}/", wait_until="networkidle", timeout=30000)
        await asyncio.sleep(4)
        shot2 = os.path.join(OUTPUT_DIR, "sidebar_fixed_root_url.png")
        await page.screenshot(path=shot2)
        print(f"Screenshot 2 (Root URL): {shot2}")
