...
async def run():
    async with async_playwright() as p:
        ...
        html_snippet = await page.evaluate("""() => {
            return {
                body_sidebar_outer_html: $('.body-sidebar').parent().html().substring(0, 1000),
                sidebar_items_html: $('.body-sidebar .sidebar-items').html(),
                all_boot_keys: Object.keys(frappe.boot?.workspace_sidebar_item || {}).slice(0, 20)
            };
        }""")
        ...
        print("\nSIDEBAR ITEMS HTML:\n", html_snippet['sidebar_items_html'])
