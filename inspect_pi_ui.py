...
async def run():
    async with async_playwright() as p:
        ...
        shot_pi = os.path.join(OUTPUT_DIR, "purchase_invoice_new_form.png")
        await page.screenshot(path=shot_pi)
        print(f"[OK] Saved PI screenshot: {shot_pi}")

        # 检查子表 columns
        cols = await page.evaluate("""() => {
            const table = document.querySelector('[data-fieldname="items"]');
            if (!table) return [];
            return Array.from(table.querySelectorAll('.grid-heading-row .grid-static-col')).map(c => c.textContent.trim());
        }""")
        print("PI Items Grid Columns:", cols)
