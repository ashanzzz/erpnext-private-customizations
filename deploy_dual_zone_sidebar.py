...
(function() {
    ...
    function toggle_section($container, force_open = null) {
        ...
        if (target_open) {
            $children.attr("data-state", "opened").stop(true, true).slideDown(150);
            $container.attr("data-state", "opened");
            $dropIcon.attr("href", "#icon-chevron-up");
            set_section_state(section_title, false);
        } else {
            $children.attr("data-state", "closed").stop(true, true).slideUp(150);
    ...
    function setup_dual_zone_handlers() {
        ...
        $(document).off("click.ashan_toggle").on(
            "click.ashan_toggle",
            ".body-sidebar .section-item .sidebar-item-control, .body-sidebar .section-item .drop-icon",
            function(e) {
                e.preventDefault();
                e.stopPropagation(); // 阻止事件冒泡到页面跳转
                const $container = $(this).closest(".sidebar-item-container");
...
with sftp.open('/tmp/ashan_cn_sidebar.js', 'wb') as f:
    f.write(UPGRADED_JS.encode('utf-8'))
sftp.close()
...
print("[OK] Deployed Dual-Zone Action Model to erpnext16 container!")
