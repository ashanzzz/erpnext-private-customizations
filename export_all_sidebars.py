...
USER = os.getenv('ERPNEXT_USERNAME', 'Administrator')
...
for sb_name in sidebars_to_export:
    ...
    if data and 'data' in data:
        ...
        file_slug = sb_name.lower().replace(' ', '_') + ".json"
        json_path = os.path.join(target_dir, file_slug)
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(doc, f, indent=1, ensure_ascii=False)
        print(f"[OK] Exported {sb_name} -> {json_path}")
