...
def evaluate_housing_fund_policy(employee, period_month, setting=None, override_mode=None):
    ...
    """
    period_month, _year, month = normalize_period_month(period_month)
    ...
    if month in rule["months"]:
        result.update({"effective_base": round(base, 2), "is_contributing": True, "decision_code": "SCHEDULED_ON", "decision_label": "季度自动缴纳", "decision_source": "公司规则", "reason": f"{month} 月属于自动缴纳月份。"})
        return result

    if rule["off_action"] == "继续缴纳":
        result.update({"effective_base": round(base, 2), "is_contributing": True, "decision_code": "OFF_MONTH_KEEP", "decision_label": "非计划月继续缴纳", "decision_source": "公司规则", "reason": f"{month} 月不是计划月，但公司配置为非计划月继续缴纳。"})
        return result

    result.update({"decision_code": "SCHEDULED_OFF", "decision_label": "季度规则停缴", "decision_source": "公司规则", "reason": f"{month} 月不是自动缴纳月份，按规则本月停缴。"})
    return result
