...
class WireTransferPicker {
                ...
                <div class="picker-filter-bar" id="wire-filter-bar">
                    <div class="picker-filter-group">
                        ...
                        <div class="picker-status-btn-group" data-filter="match_status">
                            <button type="button" class="picker-status-btn active" data-value="pending">待电汇付款</button>
                            <button type="button" class="picker-status-btn" data-value="completed">已电汇付款</button>
    ...
    bind_global_events() {
        ...
        // Filter inputs are debounced so a normal search does not issue one RPC per keystroke.
    ...
    render_kpis() {
        ...
        const cards = [
            { id: "invoice", name: "自办发票", count: this.kpis.invoice?.count || 0, sub: "待开票或录入" },
            { id: "order", name: "采购订单", count: this.kpis.order?.count || 0, sub: "自办订单" },
            { id: "receipt", name: "采购入库", count: this.kpis.receipt?.count || 0, sub: "自动入库" },
            { id: "payment", name: "电汇付款", count: this.kpis.payment?.count || 0, sub: "待电汇付款" },
        ];
                ...
                <tr>
                    <td colspan="${col_span}">
                        <div class="picker-empty-state">
                            <div class="picker-empty-text">当前暂无自办电汇采购记录，点击上方“新建自办电汇发票”即可录入。</div>
                        </div>
                    </td>
                </tr>
            `);
    ...
    open_create_wire_transfer_modal() {
        ...
        const d = new frappe.ui.Dialog({
            ...
            primary_action_label: __("生成采购单据"),
    ...
    add_wire_modal_row(dialog) {
        ...
        const tr_html = `
            <tr>
                ...
                <td><input type="text" class="ashan-smart-cell-input wire-item-spec" placeholder="规格" /></td>
                ...
                <td><input type="text" class="ashan-smart-cell-input wire-item-remarks" placeholder="备注" /></td>
