...
(function() {
    ...
    // ============================================================
    // 8. 监听 sidebar 渲染完成事件，恢复各 section 折叠状态
    // ============================================================
...
REMOTE_JS_PATH = "/home/frappe/frappe-bench/apps/ashan_cn_procurement/ashan_cn_procurement/public/js/ashan_cn_sidebar.js"
...
# 先把文件上传到 Unraid 主机临时路径
TMP_PATH = "/tmp/ashan_cn_sidebar.js"
with sftp.open(TMP_PATH, 'w') as f:
    f.write(FIXED_JS)
sftp.close()

# 再从主机复制进容器
cmd = f"docker cp {TMP_PATH} erpnext16:{REMOTE_JS_PATH}"
stdin, stdout, stderr = ssh.exec_command(cmd)
stdout.channel.recv_exit_status()
err = stderr.read().decode('utf-8')
if err:
    print("ERROR docker cp:", err)
else:
    print("[OK] sidebar.js uploaded to container")

# bench build 只构建 ashan_cn_procurement
cmd2 = "docker exec erpnext16 bash -c 'cd /home/frappe/frappe-bench && bench build --app ashan_cn_procurement 2>&1 | tail -20'"
stdin2, stdout2, stderr2 = ssh.exec_command(cmd2)
stdout2.channel.recv_exit_status()
out2 = stdout2.read().decode('utf-8')
print("[DONE] Deploy complete! Please refresh browser to test.")

# clear cache
cmd3 = "docker exec erpnext16 bash -c 'cd /home/frappe/frappe-bench && bench --site erpnext.localhost clear-cache 2>&1'"
stdin3, stdout3, stderr3 = ssh.exec_command(cmd3)
stdout3.channel.recv_exit_status()
out3 = stdout3.read().decode('utf-8')
print("clear-cache:\n", out3[:200])

ssh.close()
print("[DONE] All done! Refresh browser to test.")
