...
# Powerful script to hide legacy container and enforce pure SPA click routing
final_script = """
// 1. Inject CSS to hide legacy DOM injection containers permanently
(function() {
...
// 2. Disable legacy init function
window.init_ashan_cn_sidebar = function() { return false; };

// 3. Purge legacy container if present in DOM
(function() {
    function purge() {
        if (typeof $ !== 'undefined') {
...
// 4. Pure SPA Link Click Interceptor for Frappe Desk Sidebar
(function() {
    if (typeof $ !== 'undefined') {
        $(document).on('click', '.body-sidebar a, .desk-sidebar a, .layout-side-section a', function(e) {
            ...
            if (href.startsWith('/desk/') || href.startsWith('/app/')) {
                ...
                if (route && window.frappe && frappe.set_route) {
                    e.preventDefault();
                    e.stopPropagation();
                    console.log('[Ashan Pure SPA] Navigating smoothly without refresh to:', route);
...
// 5. Intercept /login success for seamless landing
(function() {
...
res = call_api('/api/resource/Website%20Script/Website%20Script', method='PUT', data={"javascript": final_script})
print("Updated Website Script with pure SPA interceptor and legacy DOM purge:", res is not None)
