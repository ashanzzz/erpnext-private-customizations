...
(function() {
    ...
    function get_master_sidebar() {
        if (!window.frappe || !frappe.boot || !frappe.boot.workspace_sidebar_item) return null;
    ...
    // 4. 将 masterSidebar 广播到所有 alias workspace 并保证 sidebar 渲染
    function sync_boot_sidebar_items() {
        const masterSidebar = get_master_sidebar();
        if (!masterSidebar) return;
        ...
        // 检查当前 DOM 中的 body-sidebar 是否具有完整的菜单项
        if (frappe.app && frappe.app.sidebar) {
            const itemCount = $(".body-sidebar .standard-sidebar-item").length;
            if (itemCount < 10) {
    ...
    function setup_sidebar_state_restore() {
        $(document).on("sidebar_setup", function() {
            setTimeout(sync_boot_sidebar_items, 50);
...
print("[OK] Deployed sidebar with auto render guarantee")
