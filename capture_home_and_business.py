...
async def run():
    async with async_playwright() as p:
        ...
        shot1 = os.path.join(OUTPUT_DIR, "current_desk_home.png")
        await page.screenshot(path=shot1)
        print(f"  /desk/home 截图: {shot1}")

        # 2. 访问 /desk/my-business
        print("[2] 访问 /desk/my-business ...")
        await page.goto(f"{SITE_URL}/desk/my-business", wait_until="networkidle", timeout=30000)
        await asyncio.sleep(3)
        shot2 = os.path.join(OUTPUT_DIR, "current_desk_my_business.png")
        await page.screenshot(path=shot2)
        print(f"  /desk/my-business 截图: {shot2}")
